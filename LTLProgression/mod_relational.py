...
from seaborn.utils import (categorical_order, get_color_cycle, ci_to_errsize, sort_df,
                    remove_na)
from seaborn.algorithms import bootstrap
from seaborn.palettes import color_palette, cubehelix_palette, _parse_cubehelix_args
...
class _LinePlotter(_RelationalPlotter):
    ...
    def aggregate(self, vals, grouper, units=None):
        ...
        if ci == "sd":
            ...
            cis_high = grouped.quantile(0.75)
            cis_low = grouped.quantile(0.25)
            cis = pd.DataFrame(np.c_[cis_low, cis_high], index = est.index, columns = ['low','high']).stack()
            #cis = None
