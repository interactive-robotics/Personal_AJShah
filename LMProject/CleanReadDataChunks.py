...
def ExtractCommsData(scenario):
    ...
    FinalFeatureList = ['Comms']
    
    REarth = 6371.0e3;
    for player in Playernames:
        OtherPlayers = [otherPlayer for otherPlayer in Playernames if otherPlayer!=player]
        OtherPlayer = OtherPlayers[0]
        Comm1 = rawData_sampled[player]['KEY ASSIGNMENT']
        Comm2 = rawData_sampled[OtherPlayer]['KEY ASSIGNMENT']
        Comms = np.logical_or(Comm1,Comm2).astype(int)
        FinalData[player]['Comms'] = Comms
        
    
    # Write the ownship data to the files
    for player in Playernames:
        filename = path + '/' + 'CommsData/'+scenario+'_'+player+'.pkl'
        FinalData[player].to_pickle(filename)
