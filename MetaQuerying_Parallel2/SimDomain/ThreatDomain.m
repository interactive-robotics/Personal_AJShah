classdef ThreatDomain < MissionDomain & handle
    ...
    properties
        ThreatLocations;
        ThreatThreshhold = 0.7; %The threat threshhold for the planner
        ThreatTolerance = 0.5;
        ThreatDetectionThresh = 1.0; %The threshhold for making the decision to avoid the threat
        Predicates;
    ...
    methods
            ...
            for i = [1:size(PositionalPredicates,2)]
                for j = [1:size(PositionalPredicates,1)]
                    ...
                    for k = [1:length(NewDomain.ThreatLocations)]
                        ThreatDist(k) = norm([(NewDomain.ThreatLocations(k).x-NewDomain.Waypoints(i).x), (NewDomain.ThreatLocations(k).y - NewDomain.Waypoints(i).y)]);
                    end
                    MinThreatDist = min(ThreatDist);
        ...
        function ComputePredicates(NewDomain)
            ...
            if ~isempty(NewDomain.ThreatLocations)
                ThreatPredicates = NewDomain.ComputeThreatPredicates();
                PositionalPredicates = NewDomain.ComputePositionalPredicates();
            else
                PositionalPredicates = logical(zeros(size(WaypointPredicates)));
                ThreatPredicates = logical(zeros(size(WaypointPredicates,1),0));
            end
            
            OutTable = table();
            OutTable.WaypointPredicates = logical(WaypointPredicates);
            OutTable.PositionalPredicates = logical(PositionalPredicates);
            
            OutTable.ThreatPredicates = logical(ThreatPredicates);
            
            NewDomain.Predicates = OutTable;
        end
        
        function AddRandomThreats(NewDomain, nThreats, xMax, xMin, yMax, yMin)
            for n=[1:nThreats]
                
                NewThreat.x = xMin + (xMax-xMin)*rand(1,1);
                NewThreat.y = yMin + (yMax-yMin)*rand(1,1);
                
                NewDomain.ThreatLocations = [NewDomain.ThreatLocations; NewThreat];
