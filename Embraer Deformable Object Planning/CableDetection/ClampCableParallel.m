function [ OutNode ParentNode flag ] = ClampCableParallel( InNode, manipID, clampID)
...
for i = 1:nManipulators
    if ~isempty(Manipulator(i).cable)
        if Manipulator(i).cable == cableID && i ~= manipID;
            ...
            GripLength = Cable(cableID).gripPointsPos(CoManipulatorGripPoint);
...
% Reposition the Manipulator to the clamp position
...
AddColumn = @(InputNode)ClampCable( InputNode, manipID, clampID);
