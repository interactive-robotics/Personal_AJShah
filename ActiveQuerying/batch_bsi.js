...
var LTL_Sampler4 = function(nThreats, nWaypoints) {
  /* Generate the random choices */
  var subs = DivideIntoSubsets(nWaypoints, 0.5); //Subsets for orderings
  var Threats = SubsetSampler(_.range(nThreats)); //Threat choices
  var Waypoints = SubsetSampler(_.range(nWaypoints)) // Waypoint choices
  //console.log(subs);
...
var makeModelQueryFull = function(FullData, sampler, ComplexityMeasure){
  var num_t = FullData[0].hasOwnProperty('ThreatPredicates') ? FullData[0].ThreatPredicates.length : 0;
  var num_w = FullData[0].WaypointPredicates.length;
  return function(){
    ...
    var SatCheckerTrajectory = function(data){
      ...
      var val = SatChecker(word, Formula.formula, listofatoms) ? complexity : -4*Math.log(2)*(num_t + num_w + 0.5*(num_w)*(num_w-1))
      //.console.log(Formula.orderings)
      //console.log(val)
